...
extension MainMenuActionHandler {
	...
	@objc func menuQuickCycleSub(_ sender: NSMenuItem) {
		// Kenny gennarated by Copilot in VSCode... COOL ...
		//		let _answer = dialogOKCancel(question:"Ok?", text: "Choose your answer.")
		//		if _answer == .ok {
		//			player.cycleSubtitle()
		//		}
		
		player.mpv.command(.cycle, args: ["sub"])
		// player.mpv.command(.subReload)   //测试reload
		
		//if let track = player.info.subTracks.first(where: { $0.externalFilename == player.info.currentURL.path }) {
		// player.mpv.command(.subReload, checkError: false)
		//player.mpv.command(.subRemove)
		// return
	}
	
	// Kenny20211116 Added for debug
